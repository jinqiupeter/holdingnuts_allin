...
int GameController::handleTable(Table *t)
{
	...
	// only 1 player left in Sit&Go game? close table
...
{
    if (type == SNG || type == RingGame ) { //SNT or SitAndGo
...
int GameController::tick()
{
	if (!started)
	{
        log_msg("game", "game %d is not started yet", getGameId());
		//if (type != FreezeOut)  //  MTT games must be started at scheduled time
		//	start();
		//else if (!getPlayerCount() && !getRestart())  // delete game if no players registered
		if (!getPlayerCount() && !getRestart())  // delete game if no players registered
			return -1;
